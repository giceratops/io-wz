...
public class Main {
    ...
    public static void decodeAndInflateCanvas(final WzDecoder decoder) {
        ...
        final var output = decode2(data, decoder);
    ...
    public static byte[] decode2(final byte[] input, final WzDecoder decoder) {
        var l = input.length;
        final var reader = ByteBuffer.wrap(input)
                .order(ByteOrder.LITTLE_ENDIAN);
        final var baos = new ByteArrayOutputStream();
        var read = 0;
        while (read < l) {
            var block = reader.getInt();
            read += Integer.BYTES;
            if (block > l - read || block < 0) {
                System.out.println("Critical Error - Block Size for Reading Buffer is Wrong: " + block);
                break;
            }

            for (int i = 0; i < block; i++) {
                baos.write(reader.get() ^ decoder.get(i));
                read++;
            }
        }
